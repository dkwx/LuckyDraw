...
/** 
* @ClassName: TestOfo 
* @Description: UTF-8 计算ofo的锁的撞库情况（可任意字符，任意位数,6位普通的机器就会内存溢出）
* @author daikai3 
* @date 2017年9月21日 下午1:14:05 
*  
*/
public class TestOfoV2 {
	static char[] chs = {'1','2','3','4','5','6'};
	/** 
...
//		最终查找不到的话，则从所有情况中找一个新的字符串当作下一个
	...
	* @throws 
	*/
	private static void fillAllCase() {
		getOneCase("", 0);
	}
	
	/** 
	* @Title: getOneCase 
	* @Description: 拼接到最后一位结束 
	* @param @param sb 拼接字符
	* @param @param index 当前的位数
	* @param @return    设定文件 
	* @return String    返回类型 
	* @throws 
	*/
	private static void getOneCase(String str ,int index) {
		if(index>=chs.length){
			allCase.add(str);
			return ;
		}
